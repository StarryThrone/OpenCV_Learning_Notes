...
const std::string c_window_name_in = "c_window_name_in";
const std::string c_window_name_out = "c_window_name_out";

void help(const char * argv[]) {
    std::cout << "\n"
    << "Demo: Load and smooth an image before displaying \n"
    << argv[0] <<" <path/image>\n"
    << "For example:\n"
    << argv[0] << " ../fruits.jpg\n"
    << std::endl;
...
int main(int argc, const char * argv[]) {
    ...
    // 准备窗口
    cv::namedWindow(c_window_name_in);
    cv::namedWindow(c_window_name_out);
    
    // 读取图像数据
    cv::Mat image = cv::imread(argv[1], cv::IMREAD_UNCHANGED);
    // 显示原图
    cv::imshow(c_window_name_in, image);
    
    // 模糊图片，并显示模糊后的结果
    cv::Mat out;
    cv::GaussianBlur(image, out, cv::Size(5,5), 3, 3);
    cv::GaussianBlur(out, out, cv::Size(5,5), 3, 3);
    cv::imshow(c_window_name_out, out);
